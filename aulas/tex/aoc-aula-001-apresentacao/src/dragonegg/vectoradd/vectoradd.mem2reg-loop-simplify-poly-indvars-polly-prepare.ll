...
; <label>:3                                       ; preds = %5, %.split
  %indvar = phi i64 [ %indvar.next, %5 ], [ 0, %.split ]
  %scevgep = getelementptr [1024 x float]* @h_b, i64 0, i64 %indvar
  %scevgep1 = getelementptr [1024 x float]* @h_a, i64 0, i64 %indvar
  %exitcond = icmp ne i64 %indvar, 1024
  br i1 %exitcond, label %4, label %6

; <label>:4                                       ; preds = %3
  store float 5.000000e-01, float* %scevgep1, align 4
  store float 5.000000e-01, float* %scevgep, align 4
  br label %5
...
; <label>:3                                       ; preds = %9, %.split
  %indvar = phi i64 [ %indvar.next, %9 ], [ 0, %.split ]
...
define void @check_result() #0 {
  %sum.0.reg2mem = alloca float
  %sum.0.lcssa.reg2mem = alloca float
  br label %.split

.split:                                           ; preds = %0
  store float 0.000000e+00, float* %sum.0.reg2mem
  br label %1

; <label>:1                                       ; preds = %5, %.split
  %indvar = phi i64 [ %indvar.next, %5 ], [ 0, %.split ]
  %sum.0.reload = load float* %sum.0.reg2mem
  %scevgep = getelementptr [1024 x float]* @h_c, i64 0, i64 %indvar
  %exitcond = icmp ne i64 %indvar, 1024
  store float %sum.0.reload, float* %sum.0.lcssa.reg2mem
  br i1 %exitcond, label %2, label %6

; <label>:2                                       ; preds = %1
  %3 = load float* %scevgep, align 4
  %4 = fadd float %sum.0.reload, %3
  br label %5

; <label>:5                                       ; preds = %2
  %indvar.next = add i64 %indvar, 1
  store float %4, float* %sum.0.reg2mem
  br label %1

; <label>:6                                       ; preds = %1
  %sum.0.lcssa.reload = load float* %sum.0.lcssa.reg2mem
  %7 = load %struct._IO_FILE** @stdout, align 8
  %8 = fpext float %sum.0.lcssa.reload to double
  %9 = fdiv float %sum.0.lcssa.reload, 1.024000e+03
...
; <label>:1                                       ; preds = %6, %.split
  %indvar = phi i64 [ %indvar.next, %6 ], [ 0, %.split ]
  %scevgep = getelementptr [1024 x float]* @h_c, i64 0, i64 %indvar
  %scevgep1 = getelementptr [1024 x float]* @h_b, i64 0, i64 %indvar
  %scevgep2 = getelementptr [1024 x float]* @h_a, i64 0, i64 %indvar
  %exitcond = icmp ne i64 %indvar, 1024
  br i1 %exitcond, label %2, label %7

; <label>:2                                       ; preds = %1
  %3 = load float* %scevgep2, align 4
  %4 = load float* %scevgep1, align 4
  %5 = fadd float %3, %4
